# github搜索案例练习
	## 1.如何启动我们搭建的github搜索服务器
			...
			(3).为了避免端冲突，修改webpack.json文件，将脚手架启动端口调整为8080
			(4).启动服务器：cd到github_user_server文件夹，执行：node server命令
			(5).修改Search中的请求地址为：http://localhost:3000/search/users?q=xxxxx
			(6).去webpack.confog.js中配置proxy跨域
					proxy: {
